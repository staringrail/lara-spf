...
use LaraSPF\Operators\Operations;

class CaseInsensitiveCollectionOperator implements Operations
{
    ...
    private function callbackFilterable($criteria)
    {
        $callback = function ($item) use ($criteria) {
            ...
            foreach ($criteria as $c) {
                ...
                $rule = $this->createEvaluationRule(strtolower($attribute), $c['operator'], strtolower($c['value']));
